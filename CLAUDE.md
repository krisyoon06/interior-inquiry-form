...
- 9번(예산): 문항 문구에 "스타일링의 경우 가구·소품 구매 비용" 힌트 추가 → 이후 "홈스타일링/상업공간 스타일링" 선택 시에는 예산 문항 자체를 받지 않는 것으로 재변경, 해당 힌트 문구도 제거. 9/9-1번(공사 시작일/시공 필요 부분)과 동일한 조건(홈인테리어 및 시공/상업공간 인테리어 및 시공 선택 시에만 노출)으로 묶임
- 12번 "네이버" → "네이버검색"(띄어쓰기 없음)으로 명칭 변경
- 11번(알게된 경로): UTM으로 유입 경로가 이미 확인되면 이 문항 자체를 대체함 — 아래 "UTM 유입 경로 자동화" 참고

## 배포 현황 (2026-08-20 기준)

- **저장소**: public (원래 private였으나 GitHub Pages 무료 호스팅을 위해 전환. 전환 전 `git-filter-repo`로 PDF를 커밋 히스토리에서 완전히 제거한 뒤 force-push함)
- **GitHub Pages**: https://krisyoon06.github.io/interior-inquiry-form/ (루트 접속 시 `index.html`이 `inquiry.html`로 즉시 이동). **실제 운영 중인 커스텀 도메인은 `https://inquiry.lllspace.com/`** (저장소 `CNAME` 파일로 연결됨, HTTPS 인증서 적용됨) — 실사용/라이브 테스트는 이 도메인 기준으로 할 것. github.io 쪽은 CDN(Fastly) 캐시 갱신이 커스텀 도메인보다 느릴 수 있음(직접 확인한 사례 있음, 캐시 무효화에 수 분~그 이상 소요될 수 있어 배포 확인은 커스텀 도메인으로 하는 게 더 빠름)
...
## UTM 유입 경로 자동화 (2026-09-16)

`utm_source` 쿼리 파라미터로 유입 경로를 이미 알 수 있는 트래픽은 "알게된 경로를 말씀해주세요" 문항(현재 11번 표시)을 아예 노출하지 않고, 해당 값을 자동으로 `referral` 필드에 실어 제출한다.

- 인식하는 값(화이트리스트, `inquiry.html`의 `UTM_REFERRAL_SOURCES`, 2026-09-17 개편): `naver_blog`(블로그) / `instagram_bio`(인스타그램) / `kakao_ch`(카카오채널) / `homepg`(홈페이지) / `meta_image`(메타광고_이미지) / `meta_video`(메타광고_영상) / `event`(이벤트)
  - 기존 `naver_search`(네이버검색) / `ad`(광고) 값은 제거됨. 이에 맞춰 수동 선택 라디오 옵션도 "네이버검색" → "홈페이지"로 교체됨(현재 옵션: 인스타그램/홈페이지/블로그/지인소개/광고)
- **지인소개는 UTM으로 확인이 불가능해 화이트리스트에서 의도적으로 제외** — 해당 트래픽엔 UTM을 안 붙일 것이므로 문항이 그대로 노출되어 사용자가 직접 선택하게 됨
- `utm_source`가 화이트리스트 밖 값이거나 파라미터 자체가 없으면 문항이 그대로 노출되고 기존과 동일하게 동작(회귀 없음)
- **Google Apps Script(`Code.gs`)는 의도적으로 건드리지 않음** — `referral` 컬럼에는 앞으로 UTM 원본 문자열(`instagram_bio` 등)과 기존 한글 라벨(`인스타그램` 등)이 섞여 들어올 수 있고, 시트에서의 값 정제(한글 라벨로 통일 등)는 별도로 수동 처리하기로 함
...
- [ ] Google Sheets에 남아있는 `[테스트]`/`[테스트-차단되어야함]` 접두 더미 행 정리(수동 삭제) — `[테스트] UTM시나리오1/2/3-...` 3건(2026-09-16 UTM 기능 라이브 테스트) 포함
