...
요건은 저장소에 포함된 `SNS 마케팅관리_new.pdf`에 정의된 13개 문항(필수/선택 여부, 1번 응답에 따른 10·10-1번 조건부 노출 규칙 포함)을 기준으로 하되, 실제 개발 중 다음과 같이 요건이 조정됨. 아래 문항 번호는 **PDF 원본 명세 기준 번호**이며, 현재 `inquiry.html`에 실제 표시되는 번호와는 다를 수 있음(구성원 문항 삭제로 8번 이후 문항이 한 칸씩 당겨짐 — 아래 참고):
